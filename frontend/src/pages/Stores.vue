...
<template>
  <div>
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-brand-600 to-brand-800 py-14 text-white">
      <!-- Subtle pattern overlay -->
      <div class="pointer-events-none absolute inset-0 opacity-10">
        <svg class="size-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="stores-grid" width="32" height="32" patternUnits="userSpaceOnUse">
      ...
      <div class="relative mx-auto max-w-7xl px-4 sm:px-6">
        <RouterLink
          to="/"
          class="mb-5 inline-flex items-center gap-1.5 text-sm font-medium text-brand-200 hover:text-white transition-colors"
        >
        ...
        <h1 class="text-3xl font-bold tracking-tight sm:text-4xl">Browse Stores</h1>
        <p class="mt-2 text-brand-100 sm:text-lg">Discover trusted local businesses across the Philippines.</p>
        ...
        <form class="mt-6 flex max-w-xl gap-2" @submit.prevent="onSearch">
          ...
          <button
            type="submit"
            class="shrink-0 rounded-xl bg-slate-900/70 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-900 transition-colors"
          >
            Search
          </button>
        </form>
    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      >
        ...
        <p class="mt-1 text-sm text-slate-400">Try a different search term or sector filter.</p>
        <button
          type="button"
          ...
          <div class="flex items-center gap-3 p-3">
            ...
            <div class="min-w-0">
              ...
              <p class="text-xs text-slate-400">
                {{ sectorLabels[store.sector] ?? store.city ?? "Store" }}
              </p>
            </div>
          </div>
        </RouterLink>
