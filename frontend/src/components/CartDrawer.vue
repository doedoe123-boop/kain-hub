...
import { XMarkIcon, TrashIcon, ShoppingBagIcon } from "@heroicons/vue/24/outline";
...
<template>
  ...
  <Transition name="slide">
    ...
    >
      ...
      <!-- Lines -->
      <ul v-else class="flex-1 divide-y divide-slate-100 overflow-y-auto px-1">
        <li
          ...
          <div class="flex flex-1 flex-col justify-between min-w-0">
            ...
            <div class="flex items-center justify-between mt-2">
              ...
              <div class="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 p-0.5">
                ...
                <span class="w-5 text-center text-xs font-semibold text-slate-700">
                  {{ line.quantity }}
                </span>
                <button
                  type="button"
              ...
              <div class="flex items-center gap-2">
                <span class="text-sm font-bold text-slate-900">{{ line.sub_total?.formatted }}</span>
                <button
                  type="button"
      ...
      <!-- Footer -->
      <div v-if="cart.lineCount > 0" class="border-t border-slate-100 px-5 py-4 space-y-3 bg-slate-50/60">
        <div class="flex items-center justify-between">
          <span class="text-sm text-slate-600">Subtotal</span>
          <span class="text-base font-bold text-slate-900">{{ cart.total }}</span>
        </div>
        <RouterLink
          to="/checkout"
          class="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-500 py-3.5 text-sm font-semibold text-white shadow-sm hover:bg-brand-600 hover:shadow-brand-500/25 hover:shadow-md active:bg-brand-700 transition-all"
          @click="$emit('close')"
        >
          Proceed to Checkout
        ...
        <p class="text-center text-xs text-slate-400">Taxes and delivery calculated at checkout</p>
      </div>
    </aside>
  </Transition>
</template>
